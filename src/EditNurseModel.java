...
public class EditNurseModel {
	...
	public int editPersonalInfo() {
		...
		if (!Main.dbase.containsKey(username)) {
			...
			this.user = Main.dbase.get(username); // retrieve the User object of the logged-in user
			try {
				new EditNurseController(new EditNurseModel(),
						new EditNursePersonalInfoView("Nurse Information Portal"));
				// new EditNurseController( new EditNurseModel(), new
				// EditNursePersonalInfoView("Nurse Information Portal"), new
				// NurseModel(nurseModel.getUsername(), nurseModel.getPassword(),
				// nurseModel.getName(), nurseModel.getDepartment(),
				// nurseModel.getAssignedDocUsername(), nurseModel.getAvailability() ));
