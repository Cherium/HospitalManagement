...
public class PatientController {
	...
	/**
	 * Cancel the selected view appointment on the button press; fine if late
	 * cancellation; update view
	 * 
	 * @author Sajid C
	 * 
	 */
	public void cancelAppt() {
		...
		if (model.isLateCancellation(apptIndex)) {
			...
			if (view.showOptionPane()) {
				...
				// update cancel box in view; repopulates usernameToCancel and timeToCancel
				// arrays
				view.getCancelAppt().setModel(new DefaultComboBoxModel(model.printApptList()));
	...
	public int bookAppointment() {
		...
		if (appointmentType.compareTo("Doctor Appointment") == 0) {
			String department = view.getDepartmentDropDown().getItemAt(view.getDepartmentDropDown().getSelectedIndex())
					.toString();
	...
	/**
	 * update doctor box and Appointments box when department box option changes -
	 * Booking panel
	 * 
	 * @author Sajid C
	...
	/**
	 * try to update the patients information in the database and show the error
	 * message resulting
	 * 
