	...
	public int editPersonalInfo() {
		...
		if (!Main.dbase.containsKey(username)) {
			...
			try {
				...
				new EditAdminController(new EditAdminModel(), new EditAdminPersonalInfoView("Admin Information Portal"),
						new AdminModel(adminModel.getUsername(), adminModel.getPassword(), adminModel.getName(),
								adminModel.getAvailability()));
				return 1;
			} catch (Exception e) {
				return 0;
