...
public class Schedule {
	...
	public String nextShiftsToString(LocalDateTime[] availability) {
		 ...
		 * Availability array stores 14 timings (start time and end time for every day
		 * of the week) This function get the day 'now' and appends it to the times for
		 * the day of the week (index) in the availability array. IF the start/end time
		 * are the same, then that means the person does not work that day.
		 */
		for (int i = 0; i < 14; i++) {
			int year = now.getYear();
			int month = now.getMonthValue();
			int day = now.getDayOfMonth();
			int startTime = availability[(i * 2) % (availability.length)].getHour(); // get start time, wrap around at
																						// end of week
	...
	/**
	 * return a string[] of next open slots (from current date) to print in
	 * JCombobox
	 * 
	 * @author Sajid C
	 * @param availability list of start and end times
	 * @param appointments appointment start times
	 * @return list of 100 appointment times from the current day that do not
	 *         contain any dates from appointment
	 */
	public String[] nextOpenSlots(LocalDateTime[] availability,
			HashMap<String, ArrayList<LocalDateTime>> appointments) {
		...
		for (int z = 0; z < 100; z++) {
			for (int i = 0; i < 14; i++) {
				...
				int month = now.getMonthValue();
				int day = now.getDayOfMonth();
				int startTime = availability[(i * 2) % (availability.length)].getHour(); // get start time, wrap around
																							// at end of week
				int endTime = availability[((i * 2) + 1) % (availability.length)].getHour(); // get end time.
	...
	/**
	 * Takes a list of patient usernames and a doctor username, returning all the
	 * appointments that the patients in the list have scheduled with the doctor
	 * 
	 * @param patU
	 * @param docU
	 * @return
	...
	public HashMap<String, ArrayList<LocalDateTime>> updateHashMap(ArrayList<String> patU, String docU) {
		...
		for (String p : patU) {
			// get all the appointments for the patient for a specific doctor, assuming that
			// the appointment
			// information in patient is stored as a HashMap, keys = doctor username, values
	...
	/**
	 * takes in raw availability change request data in form of array and returns a
	 * properly formatted, and updated, availability array
	 * 
	 * @author Sajid C
	 ...
	 * 
	 * @author Sajid C
	 * @param currentAvailability users availability array; output: strings in
	 *                            format HH:MM for use in view
